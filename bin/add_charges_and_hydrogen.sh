...
do
	...
	do
		...
		obabel ${system}_protein_prep.pdb -d --partialcharge $charge -xr -O  ${system}_receptor_${charge}_hydrogen.pdbqt
		obabel ${system}_ligand.sdf --partialcharge $charge -xh -O  ${system}_ligand_${charge}_hydrogen_.pdbqt
	done
done
